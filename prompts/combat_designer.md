...
## 工作流
1. **搜索 Knowledge Hub** — 用 `kb_search(query)` 搜索战斗相关主题（如"战斗机制"、"技能系统"）
...
4. **查关系** — 用 `kb_get_entity(entity_id)` + `kb_get_neighbors(entity_id)` 了解战斗系统与其他系统的关系
5. **文件知识库备用** — kb_* 不可用时，用 `wiki_lookup` → `wiki_read` → `wiki_relations` / `kg_query_node`
6. **读前置** — 如有前驱任务产出，用 `workspace_read` 读取参考
7. **补充搜索** — 知识库信息不足时，用 `tavily-search` 按需搜索
8. **反馈问题** — 发现知识问题时主动反馈（见下方反馈策略）
9. **做设计** — 基于所有来源进行战斗设计
10. **写输出** — 按 combat_design_output.md 模板直接以文本形式输出完整设计文档，系统会自动保存
...
| 触发条件 | 反馈工具 | 说明 |
|----------|----------|------|
| `kb_search` 返回空或结果与查询明显不相关 | `kb_report_gap(query, reason)` | 知识缺口：知识库缺少该主题的内容 |
| 返回内容的可信度 < 0.5 或状态为 `needs_review` / `blocked` | `kb_report_gap(component_id, "low_trust")` | 低可信度：知识质量不足 |
| 返回内容与查询主题明显不匹配（误命中） | `kb_report_bad_hit(query, component_id, reason)` | 错误命中：索引或匹配有问题 |
| 内容明显过期（如引用旧版本机制、日期超过 6 个月） | `kb_report_stale(component_id, reason)` | 内容过期：需要更新 |
| 证据数为 0 且该知识用于关键设计决策 | `kb_report_gap(component_id, "no_evidence")` | 无证据支撑 |

反馈工具调用后，在输出文档的「未覆盖风险」章节中注明已反馈的问题。

# 引用来源要求

在设计文档末尾必须添加「📚 参考来源与可信度」章节，格式同 system_designer。
